...
export default class Camera {
  ...
  lookTarget: THREE.Vector3 = new THREE.Vector3(0, 0, 0)
  currentPos: THREE.Vector3 = new THREE.Vector3(0, CAM.height, CAM.distance)
  occlusionDist: number = CAM.distance
  elapsed: number = 0
  ...
  private __dirTarget: THREE.Vector3 | null = null
  ...
  applyOcclusion(delta: number, desired: THREE.Vector3): void {
  ...
  follow(position: THREE.Vector3, direction: THREE.Vector3 | null, verticalVelocity: number): void {
    this.target.copy(position)
    this.playerVy = verticalVelocity || 0
    if (direction && direction.lengthSq() > 0.01) {
      this.playerMoving = true
      this.dirTarget = direction.clone().normalize()
    } else {
  ...
  set direction(_val: THREE.Vector3) {
    /* handled via dirTarget + follow */
  }

  get dirTarget(): THREE.Vector3 {
    return this.__dirTarget || new THREE.Vector3(0, 0, -1)
  }

  set dirTarget(val: THREE.Vector3) {
    if (!this._direction) {
