...
export type PlayerState = 'idle' | 'run' | 'jump' | 'apex' | 'fall' | 'land'
...
export interface PlayerEvents {
  'player:jump': () => void
  'player:land': (data: LandInfo) => void
}

export interface BushEntry {
  x: number
  z: number
  r: number
}
...
export default class Player {
  game: Game
  events: Events<PlayerEvents> = new Events<PlayerEvents>()
  velocity: THREE.Vector3 = new THREE.Vector3()
  horizVel: THREE.Vector2 = new THREE.Vector2(0, 0)
  ...
  jumpPressedLastFrame: boolean = false
  state: PlayerState = 'idle'
  squash: number = 0
  edgeLeanAmount: number = 0
  lastVerticalVelocity: number = 0

  mesh: THREE.Group
  trail: THREE.PointLight
  leftLeg!: THREE.Mesh
  rightLeg!: THREE.Mesh
  body: RAPIER_NS.RigidBody | null = null
  collider: RAPIER_NS.Collider | null = null
  ...
  updateSquash(delta: number): void {
  ...
  updateEdgeLean(delta: number, inputActive: boolean): void {
  ...
  updateState(inputActive: boolean): void {
    const T = TUNING
  ...
  animateLegs(moving: boolean): void {
